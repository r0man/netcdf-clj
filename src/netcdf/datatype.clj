...
;; (defrecord Datatype [uri name valid-time])

;; (defn datatype?
;;   "Returns true if arg is a datatype, otherwise false."
;;   [arg] (isa? (class arg) Datatype))

;; (defn- valid-times
;;   "Returns the valid-times of the GeoGrid."
;;   [geo-grid] (.. (.getCoordinateSystem geo-grid) getTimeAxis1D getTimeDates))

;; (defn make-datatype [uri & [name valid-time data]]
;;   (with-open [grid-dataset (dataset/open-grid-dataset uri)]
;;     (let [name (or name (first (dataset/datatype-names grid-dataset)))
;;           grid (dataset/find-geo-grid grid-dataset name)
;;           valid-time (or valid-time (first (geo-grid/valid-times grid)))]
;;       (Datatype. uri name valid-time))))

;; (read-netcdf "/home/roman/.netcdf/nww3/htsgwsfc/20100607/t00z.nc")
...
;;   [datatype] (merge (lat-axis datatype) (lon-axis datatype)))
